...
1. Copy `config/datasets/custom_rgbd.yaml` to a descriptive dataset name.
...
```bash
make validate-bag PROFILE=core DATASET=my_robot BAG=/data/my_robot
```
...
```bash
make run PROFILE=gpu DATASET=my_robot BAG=/data/my_robot
```
