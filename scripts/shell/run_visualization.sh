...
if [[ -n "$mapping" ]]; then
  [[ -n "$dataset" ]] || {
    echo "--mapping requires --dataset so the live map frame can be selected" >&2
    exit 2
  }
  mapping_profile=$(python3 "$dataset_config" "$dataset" \
    --mapping "$mapping" --get visualization.profile)
  if [[ -n "$profile" && "$profile" != "$mapping_profile" ]]; then
    echo "visualization profile '$profile' conflicts with mapping '$mapping' profile '$mapping_profile'" >&2
    exit 2
  fi
...
[[ "$profile" == hierarchical || "$profile" == khronos ]] || {
  echo "choose --mapping recorded|closed_set|open_set or --profile hierarchical|khronos" >&2
  exit 2
}
